...
# xml_path = os.path.join(package_path, 'worlds', 'Flappy_v9_RB.xml')
# xml_path = 'Flappy_v9_RB.xml'  # xml file (assumes this is in the same folder as this file)
...
if record_enable:
    viewer = mujoco_viewer.MujocoViewer(model, data, 'offscreen', width=width, height=height)  # To record video
else:
    viewer = mujoco_viewer.MujocoViewer(model, data, width=width, height=height)
...
# close
viewer.close()
if record_enable: out.release()  # done recording
if video_show_enable and record_enable: cv2.destroyWindow("video")
