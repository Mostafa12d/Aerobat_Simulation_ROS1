...
def func_create_wing_segment(p):
    ...
    id = np.concatenate((np.ones((1, p.n_blade_prox)) * 1,np.ones((1, p.n_blade_dist)) * 2,np.ones((1, p.n_blade_prox)) * 3,np.ones((1, p.n_blade_dist)) * 4,np.ones((1, p.n_blade_tail)) * 5),axis=1).reshape(p.n_blade,) #tail
    ...
    if (p.flag_use_tail_stabilizer):
        ...
        dc=np.concatenate((dc_wing,dc_tail),axis=1).reshape(3,p.n_blade)
    else:
        dc=dc_wing.reshape(3,p.n_Wagner)
    ...
    if (p.flag_use_tail_stabilizer):
        ...
        c = np.concatenate((c_wing_n, c_tail_n), axis=1).reshape(p.n_blade, )
    else:
        c = c_wing_n.reshape(p.n_blade,)
