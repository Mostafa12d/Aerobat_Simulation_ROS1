...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 22 10:59:18 2023
...
import sys, os
# sys.path.append('/Users/jackleckert/Documents/UC Berkeley/Capstone - Flapping UAV/Flapping Wing/Python/Flappy_with_aero_forces/Cython')
sys.path.append(os.path.join(os.path.dirname(__file__),'Cython')) # relative path
# print(os.path.join(os.path.dirname(__file__), 'Cython'))

'''Option to use cython'''
# from func_wing_hL_cy import func_wing_hL_f
# from func_wing_hR_cy import func_wing_hR_f
# from func_wing_rL_cy import func_wing_rL_f
# from func_wing_rR_cy import func_wing_rR_f
# from func_tail_cy import func_tail_f
# from func_create_wing_segment_cy import func_create_wing_segment_f

from symbolic_functions.func_wing_hL import func_wing_hL_f
from symbolic_functions.func_wing_hR import func_wing_hR_f
from symbolic_functions.func_wing_rL import func_wing_rL_f
from symbolic_functions.func_wing_rR import func_wing_rR_f
from symbolic_functions.func_tail import func_tail_f
from symbolic_functions.parameters import *
...
def aero(xd, R_body, xa):
    ...
    for i in range(n_blade):
        ...
        if strip_id[i] == 1:  # hL
            pos, vel, Ba = func_wing_hL_f(xd, strip_dc[0:2, i], wing_conformation,P5)
            Rw = rot_x(xd[0])
        elif strip_id[i] == 2:  # rL
            pos, vel, Ba = func_wing_rL_f(xd, strip_dc[0:2, i], wing_conformation,P5)
            Rw = rot_x(xd[1])
        elif strip_id[i] == 3:  # hR
            pos, vel, Ba = func_wing_hR_f(xd, strip_dc[0:2, i], wing_conformation,P5)
            Rw = rot_x(-xd[0])
        elif strip_id[i] == 4:  # rR
            pos, vel, Ba = func_wing_rR_f(xd, strip_dc[0:2, i], wing_conformation,P5)
            Rw = rot_x(-xd[1])
        elif strip_id[i] == 5:  # body-fixed (e.g., tail)
            pos, vel, Ba = func_tail_f(xd, strip_dc[0:3, i])
            Rw = np.eye(3)
