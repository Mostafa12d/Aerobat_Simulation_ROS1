...
''' PID Hover control; Hover at a fixed point'''
...
class Controller:
    def __init__(self,model,data):
        ...
        self.thrust_offset = mass*9.81/4
    ...
    def Control(self,model,data):
        ...
        if dt_att_ctrl >= dt_att:
            ...
            Thrust_raw = Kp_th * z_err + Ki_th * self.z_I + Kd_th * (z_err - self.z_err_prv) / dt_att + self.thrust_offset/0.35  # 0.609
            ...
            try:
                data.actuator("velocity_servo").ctrl[0] = -np.pi*2*8  # set to -29.8451302 => Drive Flapping Kinematics # data.ctrl[0]
            except:
