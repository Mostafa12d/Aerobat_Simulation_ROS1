...
glfw.init()
window = glfw.create_window(1200, 900, "Flappy Teleop Simulation (Threaded)", None, None)
glfw.make_context_current(window)
glfw.swap_interval(0) # Disable vsync to let render thread run as fast as possible (or slow if needed)
...
def physics_thread_func():
    global xa, sim_running, cage_collision_enabled
    
    cage_collision_enabled = False
    t_start_phys = time.time()
    
    print("Physics thread started")
    
    # Run physics in batches to reduce lock contention and sleep overhead
    batch_size = 10 # Run 10 steps (10ms) at a time
    
    while sim_running and data.time < simend:
        # Sync to real-time
        wall_time = time.time() - t_start_phys
        
        # If simulation is ahead of wall time, sleep
        # Only sleep if we are significantly ahead to avoid jitter from sleep overhead
        if data.time > wall_time + 0.002: # 2ms buffer
            time.sleep(data.time - wall_time)
    ...
    print("Physics thread finished")
    sim_running = False

# Start Physics Thread
phys_thread = threading.Thread(target=physics_thread_func)
phys_thread.start()

# Main Render Loop
t_start = time.time()
last_camera_time = 0
camera_rate = 30.0 # Hz - Can be higher now since it doesn't block physics

while not glfw.window_should_close(window) and sim_running:
    ...
    # Only process camera frames at a specific rate
    # Note: We use time.time() for render timing, not data.time
    current_time = time.time()
    if current_time - last_camera_time >= 1.0/camera_rate:
        ...
        try:
            ...
            # NOTE: This is the expensive operation (GPU -> CPU transfer)
            # But now it only blocks the RENDER loop, not the PHYSICS loop
...
# Cleanup
sim_running = False # Signal thread to stop if window closed
phys_thread.join() # Wait for physics thread
glfw.terminate()
