...
from symbolic_functions_faster.func_create_wing_segmentV2 import func_create_wing_segment
from symbolic_functions_faster.parameter import Simulation_Parameter
...
def aero(xa, xd_L, xd_R, R_body, p):
    ...
    strip_dc = p.strip_dc
    strip_c = p.strip_c
    strip_theta = p.strip_theta

    for i in range(p.n_blade):
        # check which wing segment this blade element index belongs to
        if strip_id[i] == 1:  # hL
            pos, vel, Ba = func_wing_hL(xd_L, strip_dc[0:2, i], p.wing_conformation_L.flatten())
            Rw = rot_x(xd_L[0])
        elif strip_id[i] == 2:  # rL
            pos, vel, Ba = func_wing_rL(xd_L, strip_dc[0:2, i], p.wing_conformation_L.flatten())
            Rw = rot_x(xd_L[1])
        elif strip_id[i] == 3:  # hR
            pos, vel, Ba = func_wing_hR(xd_R, strip_dc[0:2, i], p.wing_conformation_R.flatten())
            Rw = rot_x(-xd_R[0])
        elif strip_id[i] == 4:  # rR
            pos, vel, Ba = func_wing_rR(xd_R, strip_dc[0:2, i], p.wing_conformation_R.flatten())
            Rw = rot_x(-xd_R[1])
        elif strip_id[i] == 5:  # body-fixed (e.g., tail)
            pos, vel, Ba = func_tail(xd_L, strip_dc[0:3, i])
            Rw = np.eye(3)
    ...
    for i in range(p.n_blade):
        ...
        ua += np.dot(Ba_s[:, :, i], f_aero)
        #shape (1,8) for d2(theta5, theta6, x, y, z,roll,pitch,yaw)/dt2
